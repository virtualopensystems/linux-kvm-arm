...
static struct resource ca5s_twd_resources[] = {
	{
		.start	= A5_MPCORE_TWD,
		.end	= A5_MPCORE_TWD + 0x10,
...
{
	int err = twd_timer_register(ca5s_twd_resources,
				     ARRAY_SIZE(ca5s_twd_resources));
	if (err)
		pr_err("twd_timer_register failed %d\n", err);
}
#else
#define ca5s_twd_init	NULL
#endif
