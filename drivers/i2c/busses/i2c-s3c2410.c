...
static const struct of_device_id s3c24xx_i2c_match[] = {
	...
	{ .compatible = "samsung,s3c2440-sataphy-i2c", .data = (void *) (QUIRK_S3C2440|QUIRK_SATAPHY|QUIRK_NO_GPIO)   },
...
static u16 is_ack(struct s3c24xx_i2c *i2c)
{
	u32 time_out = i2c->tx_setup;
	while (--time_out) {
		if( readl(i2c->regs + S3C2410_IICCON) & S3C2410_IICCON_IRQPEND ) {
			if ( !(readl(i2c->regs + S3C2410_IICSTAT) & S3C2410_IICSTAT_LASTBIT))
				return true;
		}
		udelay(time_out);
        }
